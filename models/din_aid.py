...
def model_fn(features, labels, mode, params):
    ...
    user_click_aids_len = tf.constant(6, shape=[user_click_aids.get_shape().as_list()[0]])
    attention_emb = attention(aid, user_click_aids, user_click_aids_len)
    attention_emb = tf.reshape(attention_emb, shape=[-1, 16])

    # deep input dense
    dense = tf.concat([cont_feats, vector_feats, cate_emb, attention_emb], axis=1, name='dense_vector')
...
def attention_alg(params, init_emb, multi_cate_feats, single_cate_feats):
    ...
    for info in params.attention_range:
        ...
        # keys
        multi_feat = multi_cate_feats[:, info[-1][0]:info[-1][1]]
        nonzero_multi_len = tf.count_nonzero(multi_feat, axis=1)
        # keys_length
        multi_emb = tf.nn.embedding_lookup(init_emb, ids=multi_feat)  # [B, T, H]

        if match_type == 0:
            # queries
            cate_emb = tf.nn.embedding_lookup(init_emb, ids=single_cate_feats[:, info[-2]])  # [B, 1, H]
            cate_emb = tf.reshape(cate_emb, shape=[-1, params.embedding_size])  # [B, H]

            # attention
            attention_result = attention(cate_emb, multi_emb, nonzero_multi_len)  # [B, 1, H]
            attention_result = tf.reshape(attention_result, shape=[-1, params.embedding_size])  # [B, H]

        elif match_type == 1:
            # queries
            cate_emb = tf.nn.embedding_lookup(init_emb, ids=multi_cate_feats[:, info[-2][0]:info[-2][1]])  # [B, N, H]

            # attention
            attention_result = attention_multi_items(cate_emb, multi_emb, nonzero_multi_len)  # [B, N, 1, H]

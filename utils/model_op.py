...
def model_fit(model, params, train_files, predict_files):
    ...
    for epoch in range(params.epochs):
        ...
        end_time = time.time()
        print('[%s] eval-auc=%.5f\t loss=%.5f\t [%.2f s]'
              % (epoch + 1, results['auc'], results['loss'], end_time - st))
        sys.stdout.flush()
        valid_metric_list.append(results['auc'])
...
def model_save_pb(params, model):
    feature_spec = {
        ...
        'cate_feats': tf.placeholder(shape=[None, params.cate_field_size + params.multi_feats_size + params.attention_feats_size],
